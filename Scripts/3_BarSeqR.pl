...
#/path/to/script/BarSeqR.pl -org [your_organism_name] -indir /path/to/barseqR_inputs/ -exps /path/to/barseqR_inputs/FEBA_BarSeq.tsv -genes /path/to/barseqR_inputs/genes.GC -pool /path/to/barseqR_inputs/[your_sample].poolcount -noR -outdir /path/to/outputFiles/ -metadir /path/to/barseqR_inputs/ >& /path/to/outputFiles/BarSeqR.log 

#Inputs
# -org -- organism name e.g. syk6_Tn5 (This must match the organism name provided in the Metadata file)
# -indir -- input directory, must include genes, exps, and pool files (below) as well as FEBA_utils.pm FindGene.pm Perl modules. Modules can be found at https://bitbucket.org/berkeleylab/feba/src/master/
# -exps -- experiment metadata file
# -genes -- genes.gc table (You can generate a genes.gc file from an .fna assembly file from NCBI using RegionGC.pl)
# -pool -- Combined Multicodes file (combineBarSeq.pl output)
# Note that the .poolcount files must be in the “barseqR_inputs” directory and that you only have to list the first .poolcount file (prefix in first column of FEBA_BarSeq.tsv)
# -noR -- skips invoking the FEBA.R step associated with this program 
# -outdir -- Directory where output will be stored
# -metadir -- Directory where the compounds, media, and mixes metadata can be found (available at https://bitbucket.org/berkeleylab/feba/src/master/metadata/). You can include these files in the barseqR_inputs directory for simplicity.
# >& /path/to/outputFiles/<file>.log  (Optional input) -- Directory and filename for printed output from program
#Outputs
# all.poolcount file -- with columns
    # “barcode”
    # “rcbarcode” (random complement)
    # “scaffold” (organism sequence scaffold used e.g. NC_015976.1)
    # “strand” (+/-)
    # “pos” (bp position number of barcode in genome)
    # “locusId” (locus tag where barcode inserted)
    # “f” (fractional distance along gene for barcode insertion). 
    # barcode counts for each experiment specified in FEBA_BarSeq.tsv.
# exp -- copy of experiments file used as input
# genes -- copy of genes.gc file used as input
# pool -- copy of pool file used as input
# >& /path/to/outputFiles/<file>.log  (Optional) -- Directory and filename for printed output from program
####################################################################################################################################################
...

{
    ...
    foreach my $col (qw{barcode rcbarcode scaffold strand pos}) {
	...
	foreach my $file (@$filelist) {
        ...
        #The @index array is created by reading @fields as a scalar
		...
		foreach my $exp (@{ $setExps{$set} }) {
            ...
            #print STDERR "SetName is $exp->{SetName}; Set is $set; Index is $exp->{Index}; nmeta is $nmeta  \n"--used for troubleshooting
