...
<ins>**4_BarSeqProc_loadExps.py**</ins> -- Loads the all.poolcount file generated by 3_BarSeqR.pl, groups insertions according to gene locus, eliminates counts not found within a gene CDS, and eliminates genes that have fewer than three insertions in baseline sample.
...
-{baseline condition}{replicates}_in3genes.csv—File made from extracting rows from the {baseline condition}{replicate}_counts-in-genes.csv file, where >= 3 distinct transposon insertions exist for each gene in the T=0 baseline condition. This breaks apart into sets based upon replicate A,B, or C </blockquote>
